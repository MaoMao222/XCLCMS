...
var app = {
    Init: function () {
        ...
        (function (config) {
            ...
            config.opacity = 0.2;
            config.resize = true;
            config.path = XCLCMSPageGlobalConfig.RootURL + "Resources/src/js/artDialog";
        })(art.dialog.defaults);
        //XGoAjax配置
...
var ProductList = (function () {
     ...
     * 返回已选择的value数组
     */
    ProductList.prototype.GetSelectValue = function () {
        var selectVal = $(".XCLTableCheckAll").val();
        var ids = selectVal.split(',');
        if (selectVal && selectVal !== "" && ids.length > 0) {
            return ids;
     ...
     * 打开产品【修改】页面
     */
    ProductList.prototype.Update = function () {
        var $btn = $("#btnUpdate"), ids = this.GetSelectValue();
        if (ids && ids.length === 1) {
            var query = {
                handletype: "update",
     ...
     * 删除产品
     */
    ProductList.prototype.Del = function () {
        var ids = this.GetSelectValue();
        if (!ids || ids.length == 0) {
            art.dialog.tips("请至少选择一条记录进行操作！");
            return false;
